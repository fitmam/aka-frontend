...
export default function Service() {
  ...
  const fetchCompanyDescription = useCallback(async () => {
    const { data } = await http(token).get(`/company-description/${id}`);
    setCompanyDescriptionData(data.results);
  }, [id, token]);
    ...
    <div>
      ...
      <div className="flex w-full pt-32 h-screen px-5 xl:px-20 flex-col-reverse xl:flex-row">
          ...
          <div className="flex flex-col gap-10">
            <div className="flex flex-col gap-2">
              <div className="text-3xl font-bold">
                {companyDescriptionData?.title}
              </div>
              <div className="text-xl">{companyDescriptionData?.content}</div>
            </div>
            <div>
              <Link
                href={"/"}
        ...
        <div className="flex flex-1 h-full justify-center items-center z-0">
          <Image
            src={`https://res.cloudinary.com/dxnewldiy/image/upload/v1695634913/${companyDescriptionData?.picture}`}
            alt=""
            width={300}
            height={300}
