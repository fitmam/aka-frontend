...
function BusinessType() {
  ...
  async function fetchBusinessTypeById(id) {
    try {
      const { data } = await http(token).get(`/business-type/${id}`);
      setBusinessContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  const { data } = useQuery({
    queryKey: ["business-type", page, searchData, limit],
    queryFn: () => fetchBusinessType(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postBusinessType = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Tipe Bisnis",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).post("/business-type", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["business-type"] });
      setLoading(false);
      toast.success("Berhasil menambah tipe bisnis");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateBusinessType = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Update Tipe Bisnis",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/business-type/${businessTypeId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["business-type"] });
      setLoading(false);
      toast.success("Berhasil mengupdate tipe bisnis");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Tipe Bisnis",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/business-type/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["business-type"] });
      toast.success("Berhasil menghapus tipe bisnis");
      setLoading(false);
    },
    onError: (err) => {
      setLoading(false);
    ...
    <div className="flex w-full h-screen relative">
        ...
        <div className="w-full bg-[#edf0f0] p-8 flex gap-8 h-full overflow-y-scroll">
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            <div className="flex justify-between px-10 py-10 items-center w-full h-14">
              ...
              <div>
                {permissionsData?.business_type_create && (
                  <button
                    onClick={() => {
                      setOpenAddModal(!openAddModal);
                      addBusinessType.current?.resetForm();
                    }}
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Nama Tipe Bisnis</th>
                  {(permissionsData?.business_type_update ||
                    permissionsData?.business_type_delete) && (
                    <th className="text-xs w-12">Aksi</th>
                  )}
                </tr>
                {permissionsData?.business_type_read &&
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Tipe Bisnis</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            ...
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postBusinessType.mutate(values);
              setLoading(true);
            }}
            innerRef={addBusinessType}
          >
            {({
              values,
              errors,
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Tipe Bisnis</h3>
          <Formik
            initialValues={{
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Tipe Bisnis</h3>
