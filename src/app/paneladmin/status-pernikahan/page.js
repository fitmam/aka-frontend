...
function MaritalStatus() {
  ...
  const [maritalStatusContent, setMaritalStatusContent] = useState([]);
  ...
  const validateMaritalStatus = Yup.object({
  ...
  async function fetchMaritalStatusById(id) {
    try {
      const { data } = await http(token).get(`/maritalstatus/${id}`);
      setMaritalStatusContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  const showUpdateModal = (itemId) => {
    ...
    setMaritalStatusId(updatedId);
  };

  const { data } = useQuery({
    queryKey: ["maritalstatus", page, searchData, limit],
    queryFn: () => fetchMaritalStatus(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postMaritalStatus = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Status Nikah",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).post("/maritalstatus", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["maritalstatus"] });
      setLoading(false);
      toast.success("Berhasil menambah status nikah");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateBusinessType = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Update Status Nikah",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/maritalstatus/${maritalStatusId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["maritalstatus"] });
      setLoading(false);
      toast.success("Berhasil mengupdate status nikah");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Status Nikah",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/maritalstatus/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["maritalstatus"] });
      toast.success("Berhasil menghapus status nikah");
      setLoading(false);
    },
    onError: (err) => {
      setLoading(false);
    ...
    <div className="flex w-full h-screen relative">
        ...
        <div className="w-full bg-[#edf0f0] p-8 flex gap-8 h-full overflow-y-scroll">
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            <div className="flex justify-between px-10 py-10 items-center w-full h-14">
              ...
              <div>
                {permissionsData?.marital_status_create && (
                  <button
                    onClick={() => {
                      setOpenAddModal(!openAddModal);
                      addMaritalStatus.current?.resetForm();
                    }}
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Status Nikah</th>
                  {(permissionsData?.marital_status_update ||
                    permissionsData?.marital_status_delete) && (
                    <th className="text-xs w-12">Aksi</th>
                  )}
                </tr>
                {permissionsData?.marital_status_read &&
                            ...
                            {permissionsData?.marital_status_update && (
                              <button
                                onClick={() => {
                                  ...
                                  fetchMaritalStatusById(item.id);
                                }}
                                type="button"
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Status Nikah</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            validationSchema={validateMaritalStatus}
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postMaritalStatus.mutate(values);
              setLoading(true);
            }}
            innerRef={addMaritalStatus}
          >
            {({
              values,
              errors,
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Status Nikah</h3>
          <Formik
            initialValues={{
              name: maritalStatusContent.name,
            }}
            validationSchema={validateMaritalStatus}
            enableReinitialize={true}
            onSubmit={(values) => {
              setOpenEditModal(!openEditModal);
              updateBusinessType.mutate(values);
              setLoading(true);
            }}
          >
            {({
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Status Nikah</h3>
...
export default WithAuth(MaritalStatus);
