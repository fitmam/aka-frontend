...
function Education() {
  ...
  async function fetchEducationById(id) {
    try {
      const { data } = await http(token).get(`/education/${id}`);
      setEducationContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  const showUpdateModal = (itemId) => {
    ...
    setEducationId(updatedId);
  };

  const { data } = useQuery({
    queryKey: ["education", page, searchData, limit],
    queryFn: () => fetchEducation(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postEducation = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Pendidikan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).post("/education", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["education"] });
      setLoading(false);
      toast.success("Berhasil menambah pendidikan");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateEducation = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Update Pendidikan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/education/${educationId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["education"] });
      setLoading(false);
      toast.success("Berhasil mengupdate pendidikan");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Pendidikan",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/education/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["education"] });
      toast.success("Berhasil menghapus pendidikan");
      setLoading(false);
    },
    onError: (err) => {
      setLoading(false);
    ...
    <div className="flex w-full h-screen relative">
        ...
        <div className="w-full bg-[#edf0f0] p-8 flex gap-8 h-full overflow-y-scroll">
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            <div className="flex justify-between px-10 py-10 items-center w-full h-14">
              ...
              <div>
                {permissionsData?.education_create && (
                  <button
                    onClick={() => {
                      setOpenAddModal(!openAddModal);
                      addEducation.current?.resetForm();
                    }}
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Pendidikan</th>
                  {(permissionsData?.education_update ||
                    permissionsData?.education_delete) && (
                    <th className="text-xs w-12">Aksi</th>
                  )}
                </tr>
                {permissionsData?.education_read &&
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Pendidikan</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            ...
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postEducation.mutate(values);
              setLoading(true);
            }}
            innerRef={addEducation}
          >
            {({
              values,
              errors,
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Pendidikan</h3>
          <Formik
            initialValues={{
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="w-full pt-5 flex gap-4 justify-end">
                    <button
                      ...
                      className="bg-red-500 text-white p-2 rounded-md cursor-pointer w-34"
                    >
                      Tutup
                    </button>
                    <button
                      type="submit"
                      className="bg-green-500 text-white p-2 rounded-md cursor-pointer w-34"
                    >
                      Simpan
                    </button>
                  </div>
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Pendidikan</h3>
