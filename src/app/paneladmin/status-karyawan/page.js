...
function EmployeeStatus() {
  ...
  const [employeeStatusContent, setEmployeeStatusContent] = useState([]);
  ...
  const validateEmployeeStatus = Yup.object({
  ...
  const addEmployeeStatus = useRef();
  ...
  async function fetchEmployeeStatusById(id) {
    try {
      const { data } = await http(token).get(`/employee-status/${id}`);
      setEmployeeStatusContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  const showUpdateModal = (itemId) => {
    ...
    setEmployeeStatusId(updatedId);
  };

  const { data } = useQuery({
    queryKey: ["employee-status", page, searchData, limit],
    queryFn: () => fetchEmployeeStatus(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postEmployeeStatus = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Status Karyawan ",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).post("/employee-status", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["employee-status"] });
      setLoading(false);
      toast.success("Berhasil menambah status karyawan");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateEmployeeStatus = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Update Status Karyawan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/employee-status/${employeeStatusId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["employee-status"] });
      setLoading(false);
      toast.success("Berhasil mengupdate status karyawan");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Status Karyawan",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/employee-status/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["employee-status"] });
      toast.success("Berhasil menghapus status karyawan");
      setLoading(false);
    },
    onError: (err) => {
      setLoading(false);
    ...
    <div className="flex w-full h-screen relative">
        ...
        <div className="w-full bg-[#edf0f0] p-8 flex gap-8 h-full overflow-y-scroll">
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            <div className="flex justify-between px-10 py-10 items-center w-full h-14">
              ...
              <div>
                {permissionData?.employee_status_create && (
                  <button
                    onClick={() => {
                      setOpenAddModal(!openAddModal);
                      addEmployeeStatus.current?.resetForm();
                    }}
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Status Karyawan</th>
                  {(permissionData?.employee_status_update ||
                    permissionData?.employee_status_delete) && (
                    <th className="text-xs w-12">Aksi</th>
                  )}
                </tr>
                {permissionData?.employee_status_read &&
                            ...
                            {permissionData?.employee_status_update && (
                              <button
                                onClick={() => {
                                  ...
                                  fetchEmployeeStatusById(item.id);
                                }}
                                type="button"
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Status Karyawan</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            validationSchema={validateEmployeeStatus}
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postEmployeeStatus.mutate(values);
              setLoading(true);
            }}
            innerRef={addEmployeeStatus}
          >
            {({
              values,
              errors,
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Status Karyawan</h3>
          <Formik
            initialValues={{
              name: employeeStatusContent.name,
            }}
            validationSchema={validateEmployeeStatus}
            enableReinitialize={true}
            onSubmit={(values) => {
              setOpenEditModal(!openEditModal);
              updateEmployeeStatus.mutate(values);
              setLoading(true);
            }}
          >
            {({
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Status Karyawan</h3>
...
export default WithAuth(EmployeeStatus);
