...
function Religion() {
  ...
  const [religionContent, setReligionContent] = useState([]);
  ...
  const validateReligion = Yup.object({
  ...
  const addReligionContent = useRef();
  ...
  async function fetchReligionContent(id) {
    try {
      const { data } = await http(token).get(`/religion/${id}`);
      setReligionContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async function fetchReligion(
    ...
    const { data } = await http(token).get(
      "/religion?page=" + pageData + "&search=" + search + "&limit=" + limitData
    );
    return data.results;
  }
  ...
  const showUpdateModal = (itemId) => {
    ...
    setReligionId(updatedId);
  };

  const { data } = useQuery({
    queryKey: ["religion", page, searchData, limit],
    queryFn: () => fetchReligion(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postReligion = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Agama",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).post("/religion", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["religion"] });
      setLoading(false);
      toast.success("Berhasil menambah agama");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateReligion = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Update Agama",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/religion/${religionId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["religion"] });
      setLoading(false);
      toast.success("Berhasil mengupdate agama");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Agama",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/religion/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["religion"] });
    ...
    <div className="flex w-full h-screen relative">
        ...
        <div className="w-full bg-[#edf0f0] p-8 flex gap-8 h-full overflow-y-scroll">
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            <div className="flex justify-between px-10 py-10 items-center w-full h-14">
              ...
              <div>
                {permissionsData?.religion_create && (
                  <button
                    onClick={() => {
                      setOpenAddModal(!openAddModal);
                      addReligionContent.current?.resetForm();
                    }}
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Agama</th>
                  {(permissionsData?.religion_update ||
                    permissionsData?.religion_delete) && (
                    <th className="text-xs w-12">Aksi</th>
                  )}
                </tr>
                {permissionsData?.religion_read &&
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Agama</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            validationSchema={validateReligion}
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postReligion.mutate(values);
              setLoading(true);
            }}
            innerRef={addReligionContent}
          >
            {({
              values,
              errors,
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Agama</h3>
          <Formik
            initialValues={{
              name: religionContent.name,
            }}
            validationSchema={validateReligion}
            enableReinitialize={true}
            onSubmit={(values) => {
              setOpenEditModal(!openEditModal);
              updateReligion.mutate(values);
              setLoading(true);
            }}
          >
            {({
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Agama</h3>
...
export default WithAuth(Religion);
