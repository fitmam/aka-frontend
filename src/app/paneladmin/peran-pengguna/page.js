...
function UserRole() {
  ...
  async function fetchRoleById(id) {
    try {
      const { data } = await http(token).get(`/role/${id}`);
      setRoleContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async function fetchRole(
    ...
    const { data } = await http(token).get(
      "/role?page=" + pageData + "&search=" + search + "&limit=" + limitData
    );
    return data.results;
  }
  ...
  const { data } = useQuery({
    queryKey: ["role", page, searchData, limit],
    queryFn: () => fetchRole(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });
  ...
  const postRole = useMutation({
    mutationFn: (values) => {
      ...
      return http(token).post("/role", data);
    },
    onSuccess: () => {
  ...
  const updateRole = useMutation({
    mutationFn: (values) => {
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/role/${roleId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["role"] });
      setLoading(false);
  ...
  const handleDelete = useMutation({
    mutationFn: (id) => {
      return http(token).delete(`/role/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["role"] });
    ...
    <div className="flex w-full h-screen relative">
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Peran Pengguna</th>
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Peran Pengguna</h3>
          <Formik
            initialValues={{
              name: "",
            }}
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Peran Pengguna</h3>
          <Formik
            initialValues={{
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Peran Pengguna</h3>
