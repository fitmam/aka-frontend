...
function ProjectKami() {
  ...
  async function fetchProject(
    ...
    const { data } = await http(token).get(
      "/project?page=" + pageData + "&search=" + search + "&limit=" + limitData
    );
    return data.results;
  }
  ...
  async function fetchProjectById(id) {
    try {
      const { data } = await http(token).get(`/project/${id}`);
  ...
  const { data } = useQuery({
    queryKey: ["project", page, searchData, limit],
    queryFn: () => fetchProject(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postServices = useMutation({
    mutationFn: (values) => {
      ...
      return http(token).post("/project", form);
    },
    onSuccess: () => {
  ...
  const updateServices = useMutation({
    mutationFn: (values) => {
      ...
      return http(token).patch(`/project/${projectId}`, form);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["project"] });
      setLoading(false);
  ...
  const handleDelete = useMutation({
    mutationFn: (id) => {
      return http(token).delete(`/project/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["project"] });
    ...
    <div className="flex w-full h-screen relative">
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-40">Content</th>
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Project</h3>
          <Formik
            initialValues={{ title: "", content: "", order: "" }}
            validationSchema={validateService}
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postServices.mutate(values);
              setLoading(true);
            }}
            innerRef={addProject}
          >
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Update Project</h3>
          <Formik
            initialValues={{
                  ...
                  <div className="flex flex-col gap-5 pb-3">
                    <div>
                      {selectedPicture ? (
                        ...
                        <Image
                          src={`https://res.cloudinary.com/dxnewldiy/image/upload/v1695634913/${projectData.picture}`}
                          width={100}
                          height={100}
                          alt=""
                        />
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Project</h3>
...
export default WithAuth(ProjectKami);
