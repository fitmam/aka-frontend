...
function Vaccine() {
  ...
  const [vaccineContent, setVaccineContent] = useState([]);
  ...
  async function fetchVaccineById(id) {
    try {
      const { data } = await http(token).get(`/vaccine/${id}`);
      setVaccineContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async function fetchVaccine(
    ...
    const { data } = await http(token).get(
      "/vaccine?page=" + pageData + "&search=" + search + "&limit=" + limitData
    );
    return data.results;
  }
  ...
  const showUpdateModal = (itemId) => {
    ...
    setVaccineId(updatedId);
  };

  const { data } = useQuery({
    queryKey: ["vaccine", page, searchData, limit],
    queryFn: () => fetchVaccine(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postVaccine = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Vaksin",
      }).toString();
    ...
    onSuccess: () => {
      ...
      setLoading(false);
      toast.success("Berhasil menambah vaksin");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateVaccine = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vaccine"] });
      setLoading(false);
      toast.success("Berhasil mengupdate vaksin");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Vaksin",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/vaccine/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vaccine"] });
    ...
    <div className="flex w-full h-screen relative">
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Vaksin</th>
                  {(permissionsData?.vaccine_update ||
                    permissionsData?.vaccine_delete) && (
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Vaksin</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            validationSchema={validateVaccine}
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postVaccine.mutate(values);
              setLoading(true);
            }}
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Vaksin</h3>
          <Formik
            initialValues={{
              name: vaccineContent.name,
            }}
            validationSchema={validateVaccine}
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Vaksin</h3>
...
export default WithAuth(Vaccine);
