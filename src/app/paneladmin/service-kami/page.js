...
function ServiceKami() {
  ...
  async function fetchServices(
    ...
    const { data } = await http(token).get(
      "/service?page=" + pageData + "&search=" + search + "&limit=" + limitData
    );
    return data.results;
  }
  ...
  const { data } = useQuery({
    queryKey: ["services", page, searchData, limit],
    queryFn: () => fetchServices(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });
  ...
  const postServices = useMutation({
    mutationFn: (values) => {
      ...
      return http(token).post("/service", form);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["services"] });
      setLoading(false);
      toast.success("Berhasil menambah services");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateServices = useMutation({
    mutationFn: (values) => {
      ...
      return http(token).patch(`/service/${servicesId}`, form);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["services"] });
      setLoading(false);
      toast.success("Berhasil mengupdate services");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: (id) => {
      return http(token).delete(`/service/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["services"] });
      toast.success("Berhasil menghapus services");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
    ...
    <div className="flex w-full h-screen relative">
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Service</h3>
          <Formik
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Update Services</h3>
          <Formik
            initialValues={{
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Service</h3>
...
export default WithAuth(ServiceKami);
