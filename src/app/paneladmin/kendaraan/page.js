...
function VehicleType() {
  ...
  const validateVehicleType = Yup.object({
  ...
  async function fetchVehicleTypeById(id) {
    try {
      const { data } = await http(token).get(`/vehicle-type-list/${id}`);
      setVehicleContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  const showUpdateModal = (itemId) => {
    ...
    setVehicleTypeId(updatedId);
  };

  const { data } = useQuery({
    queryKey: ["vehicle-type-list", page, searchData, limit],
    queryFn: () => fetchVehicleType(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postVehicleType = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Kendaraan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).post("/vehicle-type-list", data);
    },
    onSuccess: () => {
  ...
  const updateVehicleType = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Update Kendaraan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/vehicle-type-list/${vehicleTypeId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vehicle-type-list"] });
      setLoading(false);
      toast.success("Berhasil mengupdate kendaraan");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Kendaraan",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/vehicle-type-list/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vehicle-type-list"] });
    ...
    <div className="flex w-full h-screen relative">
        ...
        <div className="w-full bg-[#edf0f0] p-8 flex gap-8 h-full overflow-y-scroll">
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            <div className="flex justify-between px-10 py-10 items-center w-full h-14">
              ...
              <div>
                {permissionsData?.vehicle_type_create && (
                  <button
                    onClick={() => {
                      setOpenAddModal(!openAddModal);
                      addVehicleType.current?.resetForm();
                    }}
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Kendaraan</th>
                  {(permissionsData?.vehicle_type_update ||
                    permissionsData?.vehicle_type_delete) && (
                    <th className="text-xs w-12">Aksi</th>
                  )}
                </tr>
                {permissionsData?.vehicle_type_read &&
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Kendaraan</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            validationSchema={validateVehicleType}
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postVehicleType.mutate(values);
              setLoading(true);
            }}
            innerRef={addVehicleType}
          >
            {({
              values,
              errors,
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Kendaraan</h3>
          <Formik
            initialValues={{
              name: vehicleContent.name,
            }}
            validationSchema={validateVehicleType}
            enableReinitialize={true}
            onSubmit={(values) => {
              setOpenEditModal(!openEditModal);
              updateVehicleType.mutate(values);
              setLoading(true);
            }}
          >
            {({
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Kendaraan</h3>
...
export default WithAuth(VehicleType);
