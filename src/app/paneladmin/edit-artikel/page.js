...
function EditArtikel() {
  ...
  const fetchArticle = useCallback(async () => {
    ...
    setCategoryId(data.results.categoryId);
  }, [id, token]);
  ...
  }, [primary, secondary, colorData, dispatch, token, articleData, id]);
  ...
  const updateArticle = useMutation({
    mutationFn: (values) => {
      ...
      return http(token).patch(`/article/${id}`, form);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["article"] });
      setLoading(false);
      toast.success("Berhasil mengedit artikel");
      router.push("/paneladmin/artikel");
    },
    onError: (err) => {
      setLoading(false);
          ...
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            ...
            <div className="px-10 pb-10">
              {selectedPicture ? (
                ...
                <Image
                  src={`https://res.cloudinary.com/dxnewldiy/image/upload/v1695634913/${articleData.picture}`}
                  width={200}
                  height={200}
                  alt=""
                ></Image>
                            ...
                            {categoryData?.data?.map((item) => {
                              return (
                                <option
                                  key={item.id}
                                  value={item.id}
                                  selected={articleData.category_id}
                                >
                                  {item.title}
                                </option>
                              );
...
export default WithAuth(EditArtikel);
