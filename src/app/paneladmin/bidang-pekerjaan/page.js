...
function FieldOfWork() {
  ...
  const [fieldOfWorkContent, setFieldOfWorkContent] = useState([]);
  ...
  const validateBusinessType = Yup.object({
  ...
  async function fetchFieldOfWorkById(id) {
    try {
      const { data } = await http(token).get(`/fieldofwork/${id}`);
      setFieldOfWorkContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async function fetchFieldOfWork(
    pageData = page,
    search = searchData,
    limitData = limit
  ) {
  ...
  const showUpdateModal = (itemId) => {
    ...
    setFieldOfWorkId(updatedId);
  };

  const { data } = useQuery({
    queryKey: ["fieldofwork", page, searchData, limit],
    queryFn: () => fetchFieldOfWork(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postFieldOfWork = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Bidang Pekerjaan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).post("/fieldofwork", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["fieldofwork"] });
      setLoading(false);
      toast.success("Berhasil menambah bidang pekerjaan");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateFieldOfWork = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Update Bidang Pekerjaan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/fieldofwork/${fieldOfWorkId}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["fieldofwork"] });
      setLoading(false);
      toast.success("Berhasil mengupdate bidang pekerjaan");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Bidang Pekerjaan",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/fieldofwork/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["fieldofwork"] });
      toast.success("Berhasil menghapus bidang pekerjaan");
      setLoading(false);
    },
    onError: (err) => {
      setLoading(false);
    ...
    <div className="flex w-full h-screen relative">
        ...
        <div className="w-full bg-[#edf0f0] p-8 flex gap-8 h-full overflow-y-scroll">
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            <div className="flex justify-between px-10 py-10 items-center w-full h-14">
              ...
              <div>
                {permissionsData?.field_of_work_create && (
                  <button
                    onClick={() => {
                      setOpenAddModal(!openAddModal);
                      addFieldOfWork.current?.resetForm();
                    }}
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Bidang Pekerjaan</th>
                  {(permissionsData?.field_of_work_update ||
                    permissionsData?.field_of_work_delete) && (
                    <th className="text-xs w-12">Aksi</th>
                  )}
                </tr>
                {permissionsData?.field_of_work_read &&
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Bidang Pekerjaan</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            validationSchema={validateBusinessType}
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postFieldOfWork.mutate(values);
              setLoading(true);
            }}
            innerRef={addFieldOfWork}
          >
            {({
              values,
              errors,
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Bidang Pekerjaan</h3>
          <Formik
            initialValues={{
              name: fieldOfWorkContent.name,
            }}
            validationSchema={validateBusinessType}
            enableReinitialize={true}
            onSubmit={(values) => {
              setOpenEditModal(!openEditModal);
              updateFieldOfWork.mutate(values);
              setLoading(true);
            }}
          >
            {({
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Bidang Pekerjaan</h3>
...
export default WithAuth(FieldOfWork);
