...
function NeededPosition() {
  ...
  const [neededPosition, setNeededPosition] = useState("");
  const [loading, setLoading] = useState(false);
  const [neededPositionContent, setNeededPositionContent] = useState([]);
  ...
  const validateNeededPosition = Yup.object({
  ...
  const addNeededPosition = useRef();
  ...
  async function fetchNeededPositionById(id) {
    try {
      const { data } = await http(token).get(`/needed-position/${id}`);
      setNeededPositionContent(data.results);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async function fetchNeededPosition(
    pageData = page,
    search = searchData,
    limitData = limit
  ) {
    const { data } = await http(token).get(
      "/needed-position?page=" +
        pageData +
        "&search=" +
        search +
        "&limit=" +
  ...
  const showUpdateModal = (itemId) => {
    ...
    setNeededPosition(updatedId);
  };

  const { data } = useQuery({
    queryKey: ["needed-position", page, searchData, limit],
    queryFn: () => fetchNeededPosition(page, searchData, limit),
    staleTime: 10 * 60 * 1000,
    cacheTime: 60 * 60 * 1000,
  });

  const postNeededPosition = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Tambah Posisi Dibutuhkan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).post("/needed-position", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["needed-position"] });
      setLoading(false);
      toast.success("Berhasil menambah Posisi Dibutuhkan");
    },
    onError: (err) => {
      setLoading(false);
  ...
  const updateNeededPosition = useMutation({
    mutationFn: async (values) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Update Posisi Dibutuhkan",
      }).toString();
      await http(token).post("/user-log", logData);
      const data = new URLSearchParams(values).toString();
      return http(token).patch(`/needed-position/${neededPosition}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["needed-position"] });
      setLoading(false);
      toast.success("Berhasil mengupdate Posisi Dibutuhkan");
    },
    onError: (err) => {
      setLoading(false);
      toast.error(err?.response?.data?.message);
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const logData = new URLSearchParams({
        user_name: currentUser.first_name,
        role_id: currentUser.role_id,
        crud_activites: "Hapus Posisi Dibutuhkan",
      }).toString();
      await http(token).post("/user-log", logData);
      return http(token).delete(`/needed-position/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["needed-position"] });
      toast.success("Berhasil menghapus Posisi Dibutuhkan");
      setLoading(false);
    },
    onError: (err) => {
      setLoading(false);
    ...
    <div className="flex w-full h-screen relative">
          ...
          <div className="w-full h-screen bg-white rounded-md overflow-auto">
            <div className="flex justify-between px-10 py-10 items-center w-full h-14">
              <div className="flex items-center gap-2">
                ...
                Data Posisi Dibutuhkan
              </div>
              <div>
                {permissionsData?.needed_position_create && (
                  <button
                    onClick={() => {
                      setOpenAddModal(!openAddModal);
                      addNeededPosition.current?.resetForm();
                    }}
            ...
            <div className="px-10 py-10 max-w-xs md:max-w-none md:overflow-visible overflow-x-auto">
              <table className="w-full">
                <tr className="h-16 bg-[#E0F4FF]">
                  ...
                  <th className="text-xs w-48">Posisi Dibutuhkan</th>
                  {(permissionsData?.needed_position_update ||
                    permissionsData?.needed_position_delete) && (
                    <th className="text-xs w-12">Aksi</th>
                  )}
                </tr>
                {permissionsData?.needed_position_read &&
                            ...
                            {permissionsData?.needed_position_update && (
                              <button
                                onClick={() => {
                                  ...
                                  fetchNeededPositionById(item.id);
                                }}
                                type="button"
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Tambah Posisi Dibutuhkan</h3>
          <Formik
            initialValues={{
              name: "",
            }}
            validationSchema={validateNeededPosition}
            onSubmit={(values) => {
              setOpenAddModal(!openAddModal);
              postNeededPosition.mutate(values);
              setLoading(true);
            }}
            innerRef={addNeededPosition}
          >
            {({
              values,
              errors,
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Edit Posisi Dibutuhkan</h3>
          <Formik
            initialValues={{
              name: neededPositionContent.name,
            }}
            validationSchema={validateNeededPosition}
            enableReinitialize={true}
            onSubmit={(values) => {
              setOpenEditModal(!openEditModal);
              updateNeededPosition.mutate(values);
              setLoading(true);
            }}
          >
            {({
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg pb-5">Hapus Posisi Dibutuhkan</h3>
...
export default WithAuth(NeededPosition);
